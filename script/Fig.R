...
for(i in 1:length(labels)){
  cols[i]<-unique(sp_food_coltp0$col7[labels1[i]==sp_food_coltp0$foodhabit7])
}
cols
...
#L_lemairii(eff)対A_dewindti(cau)
summary(SmapInterOnly[[5]]>0)
Smap_TF_list[[5]]<-rbind("Positive",sum(SmapInterOnly[[5]]>0)/38)
...
for(i in 19:length(SmapInterOnly)){
  Smap_TF_list[[i]]<-apply(SmapInterOnly[[i]],2,forEdgeGrade)
}
...
rownames(Smap_TF_list_t_for_col)[111]<-"L_attenuatus"
...
rownames(Smap_TF_list_t_bind)[33]<-"A_dewindti"
rownames(Smap_TF_list_t_bind)[111]<-"L_attenuatus"
...
Smap_cum_sums<-list()
for(i in 1:4){
Smap_cum_sums[[i]]<-abs(apply(SmapInterOnly[[i]],2,sum))
}

Smap_cum_sums[[5]]<-abs(sum(SmapInterOnly[[5]]))

for(i in 6:17){
  Smap_cum_sums[[i]]<-abs(apply(SmapInterOnly[[i]],2,sum))
}

Smap_cum_sums[[18]]<-abs(sum(SmapInterOnly[[18]]))

for(i in 19:length(SmapInterOnly)){
  Smap_cum_sums[[i]]<-abs(apply(SmapInterOnly[[i]],2,sum))
}

names(Smap_cum_sums)<-names(SmapInterOnly)

Smap_cum_sum<-list()
for(i in 1:length(Smap_cum_sums)){
  Smap_cum_sums[[i]]<-cbind(rep(names(Smap_cum_sums)[i],length(Smap_cum_sums[[i]])),names(Smap_cum_sums[[i]]),Smap_cum_sums[[i]])
}

#L_lemairii(eff)-A_dewindti(cau)
Smap_cum_sums$L_lemairii<-cbind(Smap_cum_sums$L_lemairii[,1],"A_dewindti",Smap_cum_sums$L_lemairii[,2])
#L_labiatus(eff)-L_attenuatus(cau)
Smap_cum_sums$L_labiatus<-cbind(Smap_cum_sums$L_labiatus[,1],"L_attenuatus",Smap_cum_sums$L_labiatus[,2])

Smap_abs_means<-list()
for(i in 1:4){
  Smap_abs_means[[i]]<-apply(apply(SmapInterOnly[[i]],2,abs),2,mean)
}

Smap_abs_means[[5]]<-mean(abs(SmapInterOnly[[5]]))

for(i in 6:17){
  Smap_abs_means[[i]]<-apply(apply(SmapInterOnly[[i]],2,abs),2,mean)
}

Smap_abs_means[[18]]<-mean(abs(SmapInterOnly[[18]]))

for(i in 19:length(SmapInterOnly)){
  Smap_abs_means[[i]]<-apply(apply(SmapInterOnly[[i]],2,abs),2,mean)
}

names(Smap_abs_means)<-names(SmapInterOnly)

names(Smap_abs_means$L_lemairii)<-"A_dewindti"

names(Smap_abs_means$L_labiatus)<-"L_attenuatus"


Smap_abs_mean<-list()
for(i in 1:length(Smap_abs_means)){
  Smap_abs_mean[[i]]<-cbind(rep(names(Smap_abs_means)[i],length(Smap_abs_means[[i]])),names(Smap_abs_means[[i]]),Smap_abs_means[[i]])
}


#L_lemairii(eff)-A_dewindti(cau)
Smap_abs_mean[[5]]<-cbind(Smap_abs_mean[[5]][,1],Smap_abs_mean[[5]][,2],Smap_abs_mean[[5]][,3])
#L_labiatus(eff)-L_attenuatus(cau)
...
Smap_abs_mean_with_ind<-NULL
for(i in 1:length(Smap_abs_mean_with_inds)){
...
Smap_cum_sums_with_inds<-list()
for(i in 1:length(Smap_cum_sums)){
  Smap_cum_sums_with_inds[[i]]<-cbind(Smap_cum_sums[[i]],paste(Smap_cum_sums[[i]][,1],Smap_cum_sums[[i]][,2],sep="_"))
}

Smap_cum_sum_with_ind<-NULL
for(i in 1:length(Smap_cum_sums_with_inds)){
  Smap_cum_sum_with_ind<-rbind(Smap_cum_sum_with_ind,Smap_cum_sums_with_inds[[i]])
}

colnames(Smap_cum_sum_with_ind)<-c("effect","cause","Smap_cum_sum","effect_cause")
Smap_cum_sum_with_ind<-as.data.frame(Smap_cum_sum_with_ind)
...
#以下なぜかedge.arrow.width=as.numeric(correlation_Hori$ratio)*2でエラー
#pdf("Hori_correlation_map_aggressive_behavior_edge.pdf")
...
#     vertex.label.family="Times",
#     vertex.label.font=4,
#     edge.width=as.numeric(correlation_Hori$ratio)*2,
#     edge.arrow.width=as.numeric(correlation_Hori$ratio)*2,
#     edge.arrow.size=0.2,
#     vertex.label.color="black",
#     vertex.color=Hori_graph_col,
#     vertex.label.dist=0,
...
plot(Agg_hori_fig,layout=lay.crctp0_Hori,
     ...
     vertex.label.font=4,
     edge.width=(correlation_Hori$ratio)*2,
     edge.arrow.size=(log(correlation_Hori$ratio)-min(log(correlation_Hori$ratio))+1)*0.1,
     vertex.label.color="black",
     vertex.color=Hori_graph_col,
     vertex.label.dist=0,
     #c("orange","green","red","green","red",
...
ggsave("FigS4a.pdf",width = 10, height = 10, units = "in", device = cairo_pdf)
...
ggsave("FigS4b.pdf",width = 10, height = 10, units = "in", device = cairo_pdf)
...
degree_dist_All_in_col_data<-cbind(degree_dist_All_col_data,pop_mean_in)
...
legend_data_AllPos <- data.frame(
  resultAllPosFood = unique(corResultCauseAllEffectPos$resultCauseAllEffectPosFood),
  resultAllPosCol = unique(corResultCauseAllEffectPos$resultCauseAllEffectPosCol)
)
