...
def organize_degree_sequences(deg_dir):
    fnV = [file for file in os.listdir(deg_dir) if file.split('.')[-1] in ['txt', 'csv']]
...
def analyze_degree_sequences(deg_dir, analysis, overwrite=False):
...
def categorize_networks(df, permissive=False):
    ...
    if permissive:
        hyps = pd.DataFrame(columns = ['Strongest', "Strong", "Weak", "Weakest",
                                       "Super_Weak", "Strong_Any", "Weak_Any", "Weakest_Any",
    ...
    for i, dataset in enumerate(unique_datasets):
        ...
        if permissive:
            ...
            hyps.loc[dataset]['Strong_Any'] = S[0]
